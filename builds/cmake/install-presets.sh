...
#
###############################################################################
# Script managing the build and installation of libbitcoin-node and its dependencies.
#
# Script options:
...
main()
{
    ...
    if [[ "${BUILD_PRESET}" == "nix-gnu-debug-static" ]]; then
        msg_verbose "BUILD_PRESET value 'nix-gnu-debug-static'."
        BUILD_CONFIG="debug"
        BUILD_LINK="static"
    elif [[ "${BUILD_PRESET}" == "nix-gnu-debug-shared" ]]; then
        msg_verbose "BUILD_PRESET value 'nix-gnu-debug-shared'."
        BUILD_CONFIG="debug"
        BUILD_LINK="dynamic"
    elif [[ "${BUILD_PRESET}" == "nix-gnu-release-static" ]]; then
        msg_verbose "BUILD_PRESET value 'nix-gnu-release-static'."
        BUILD_CONFIG="release"
        BUILD_LINK="static"
    elif [[ "${BUILD_PRESET}" == "nix-gnu-release-shared" ]]; then
        msg_verbose "BUILD_PRESET value 'nix-gnu-release-shared'."
        BUILD_CONFIG="release"
        BUILD_LINK="dynamic"
    else
    ...
    build_preset "libbitcoin-database" "builds/cmake" "." "${PARALLEL}" "${libbitcoin_database_OPTIONS[@]}" "${CONFIGURE_OPTIONS_CMAKE[@]}"
    ...
    build_preset "libbitcoin-network" "builds/cmake" "." "${PARALLEL}" "${libbitcoin_network_OPTIONS[@]}" "${CONFIGURE_OPTIONS_CMAKE[@]}"
    install_cmake "libbitcoin-network" "."
    if [[ "${BUILD_POST_INSTALL_CLEAN}" == "yes" ]]; then
        clean_cmake "libbitcoin-network" "."
    fi
    export CPPFLAGS="${SAVE_CPPFLAGS}"
    ...
    build_preset "libbitcoin-node" "builds/cmake" "." "${PARALLEL}" "${libbitcoin_node_OPTIONS[@]}" "${CONFIGURE_OPTIONS_CMAKE[@]}"
    if ! [[ "${BUILD_SKIP_TESTS}" == "yes" ]]; then
        test_cmake "libbitcoin-node" "." "${PARALLEL}"
    fi
    install_cmake "libbitcoin-node" "."
    if [[ "${BUILD_POST_INSTALL_CLEAN}" == "yes" ]]; then
        clean_cmake "libbitcoin-node" "."
    fi
    export CPPFLAGS="${SAVE_CPPFLAGS}"
...
display_toolchain_variables()
{
    ...
    msg "CMAKE_PREFIX_PATH               : ${CMAKE_PREFIX_PATH}"
    msg "CMAKE_INSTALL_PREFIX            : ${CMAKE_INSTALL_PREFIX}"
    msg "CMAKE_INCLUDE_PATH              : ${CMAKE_INCLUDE_PATH}"
    msg "CMAKE_LIBRARY_PATH              : ${CMAKE_LIBRARY_PATH}"
}
...
display_constants()
{
    ...
    msg "libbitcoin_database_OWNER       : ${libbitcoin_database_OWNER}"
    msg "libbitcoin_database_TAG         : ${libbitcoin_database_TAG}"

    msg "libbitcoin_network_OWNER        : ${libbitcoin_network_OWNER}"
    msg "libbitcoin_network_TAG          : ${libbitcoin_network_TAG}"

    msg "libbitcoin_node_OWNER           : ${libbitcoin_node_OWNER}"
    msg "libbitcoin_node_TAG             : ${libbitcoin_node_TAG}"
}

help()
{
    msg "Script managing the build and installation of libbitcoin-node and its dependencies."
    msg ""
    msg "Script options:"
