...
:main
    ...
    call :source_github "!libbitcoin_database_OWNER!" "libbitcoin-database" "!libbitcoin_database_TAG!"
    if %ERRORLEVEL% neq 0 (
        exit /b %ERRORLEVEL%
    )
    ...
    call :source_github "!libbitcoin_network_OWNER!" "libbitcoin-network" "!libbitcoin_network_TAG!"
    if %ERRORLEVEL% neq 0 (
        exit /b %ERRORLEVEL%
    )
    ...
    call :source_github "!libbitcoin_node_OWNER!" "libbitcoin-node" "!libbitcoin_node_TAG!"
    if %ERRORLEVEL% neq 0 (
        exit /b %ERRORLEVEL%
    )
    ...
    if not "!BUILD_SKIP_TESTS!" == "yes" (
        call :build_msbuild "libbitcoin-node" "builds\msvc\%proj_version%" "libbitcoin-node-test" "!REMAPPED_PARAMS!"
        if %ERRORLEVEL% neq 0 (
            exit /b %ERRORLEVEL%
        )
    )
...
:display_constants
    ...
    call :msg "libbitcoin_database_OWNER       : !libbitcoin_database_OWNER!"
    call :msg "libbitcoin_database_TAG         : !libbitcoin_database_TAG!"
    call :msg "libbitcoin_network_OWNER        : !libbitcoin_network_OWNER!"
    call :msg "libbitcoin_network_TAG          : !libbitcoin_network_TAG!"
    call :msg "libbitcoin_node_OWNER           : !libbitcoin_node_OWNER!"
    call :msg "libbitcoin_node_TAG             : !libbitcoin_node_TAG!"
    exit /b %ERRORLEVEL%
