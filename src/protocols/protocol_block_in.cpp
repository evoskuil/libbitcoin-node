...
protocol_block_in::protocol_block_in(node_interface& network,
    channel::ptr channel, full_chain& blockchain)
  : protocol<protocol_timer>(network, channel, perpetual_timer, NAME),
...
void protocol_block_in::send_get_blocks(const hash_digest& stop_hash)
{
    ...
    if (chain_top != null_hash && chain_top == last_locator_top_.load())
        return;

    const auto heights = chain::block::locator_heights(top_height());
